<template>
  <div class="container">
    <el-container>
      <el-table
        :data="data.dataList"
        :cell-style="{ 'max-height': '200px' }"
        :border="true"
        :stripe="true"
        :max-height="data.tableHeight"
      >
        <el-table-column type="index" lable="#" width="40px" :fixed="true" :align="'center'" />
        <el-table-column prop="level" label="类型" width="100px" :align="'center'" />
        <el-table-column prop="message" label="内容" :align="'center'">
          <template #default="scope"><text-trunvated :data="scope.row.message" :showRow="2" /></template>
        </el-table-column>
        <el-table-column prop="createAt" label="日志时间" width="180px" :formatter="dataFormat" :align="'center'" />
      </el-table>
