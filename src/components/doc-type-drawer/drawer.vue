  ...
  >
    <el-form :model="data.form">
      <el-form-item label="文档类型" label-width="80px">
        <el-input v-model="data.form.name" autocomplete="off" placeholder="请输入文档类型" />
      </el-form-item>
      <el-form-item label="内容类型" label-width="80px">
        <el-checkbox-group v-model="data.form.contents">
          <el-checkbox v-for="item in contents" :label="item.name" :key="item.name" :border="true" :value="item.name" />
        </el-checkbox-group>
      </el-form-item>
    ...
    <div class="dialog-footer">
      ...
      <el-button type="primary" :loading="data.loading" @click="handleClose">{{
        data.loading ? '提交中 ...' : '确定'
      }}</el-button>
    </div>
  </el-drawer>
...
import { Content, DocType } from '@/types/doc';
...
getContentList().then((res) => {
  if (res.data?.length) {
    ...
    contents.push(...[{ name: '知识点' }, { name: '问答题' }]);
  }
});

//上传成功
const handleAvatarSuccess = (res: { status: boolean; message: string; data: string }) => {
  if (res.status) {
